...
void asfApp::updateScene() {
  ...
  if (skelload){
    ...
    m_kf_play_pos = glm::mod( m_kf_play_pos, float( workPoses.size()-1 ) );
...
//      printf("bang!\n");
  ...
  ImGui::SliderFloat("KF Sequence Position",&m_kf_play_pos,-0.f , workPoses.size()-1, "%.5f", 1.0f);
  ...
  if(ImGui::Button("Refresh Curves")){
    ...
    boneCurveMap.clear();
     ...
     if (workPoses.size()>0){
       ...
       //TODO: Sort out handling the root
